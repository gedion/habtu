...
//Product version details:2.1.0, 01-28-2009Updated and tested on IE, Firefox, Opera, Safari and Google Chrome
...
AmharicPhoneticKeyPress= function (pressEvent, amharic){
 var keyCode = 0
...
if(controlKey) return true;
 ...
 var lastCharPlain = amharic.innerHTML.charCodeAt(startPos-1);
 ...
 if (keyCode != 16 && keyCode != 8 && keyCode != 13 && keyCode != 0 && keyCode != 32)//&& keyCode != 116) //shift)
 {
   ...
   var tempExtraLetter = amharic.innerHTML.substring(0,amharic.innerHTML.length - 1);
   ...
   if (consonantRetVal != null && consonantRetVal.length > 0)
   {
       ...
       if( amharicValue == "h" && shiftKey == false)
       {
           ...
           amharic.innerHTML = amharic.innerHTML.substring(0, tempStartPos-1)
               + String.fromCharCode(hconsValue )
               + amharic.innerHTML.substring(startPos, amharic.innerHTML.length);
       ...
       {
           amharic.innerHTML = amharic.innerHTML + String.fromCharCode(4959); // 135F = Tebbeq
           newPos = newPos + 1;    
       
       }
       else
       {
           amharic.innerHTML = amharic.innerHTML.substring(0, startPos)
               + consonantRetVal
               + amharic.innerHTML.substring(endPos, amharic.innerHTML.length);
   ...
   {
       amharic.innerHTML = amharic.innerHTML + String.fromCharCode(4964);
       newPos = newPos + 1;    
       
   } else if( amharicValue == ":" )
   {
       amharic.innerHTML = amharic.innerHTML + String.fromCharCode(4961);
       newPos = newPos + 1;    
   }

   var vowelCharacterChar = String.fromCharCode(vowelCharacter);
       //var cursorIndex = amharic.innerHTML.indexOf("|");
       //cursor.text = "" + cursorIndex; 

   var lastCharEtymology = amharic.innerHTML.charCodeAt(startPos-1);

   if ( offset != 0 )
   {//?
       
       if ( amharic.innerHTML.length < 1 )
       {
           amharic.innerHTML = amharic.innerHTML + vowelCharacterChar ;
                   newPos = newPos + 1;    

           //pressEvent.keyCode = 4773;
       }else if(amharic.innerHTML.charCodeAt(startPos-1) == 32 || lastCharPlain == 4959)  // if tebeq
       {
           amharic.innerHTML = amharic.innerHTML + vowelCharacterChar ;
                   newPos = newPos + 1;    
       ...
       else if (lastCharEtymology  == 4883 || lastCharEtymology == 4683 || lastCharEtymology == 4803 || lastCharEtymology == 4787 || lastCharEtymology == 4747)
       {
           ...
           if ( amharicValue == "u") lastCharEtymology = lastCharEtymology + 2;
               ...
               amharic.innerHTML = amharic.innerHTML.substring(0, startPos-1)
       ...
       {
           var lastchar = amharic.innerHTML.charCodeAt(startPos-1) + offset;
   ...
   //Output.value = amharic.innerHTML.charCodeAt(startPos-1);
   if ( amharicValue == "w" && shiftKey == true )//amharicValue == "/") // keycode == 191
   {//4775
       var lastchar2 = amharic.innerHTML.charCodeAt(startPos-1);

       if (amharic.innerHTML.length < 1) 
       {
           lastCharModified = 4775;
           amharic.innerHTML = String.fromCharCode(4775)
           newPos = newPos + 1;
       }
       ...
       if( ! (lastchar2 == 4845 || lastchar2 == 4813 || lastchar2 == 4933 || lastchar2 == 4768  || lastchar2 == 4821) )
       {
           ...
           if (isRootLetter(lastchar2))
           {
               ...
               //amharic.innerHTML = amharic.innerHTML.substring(0, amharic.innerHTML.charCodeAt(startPos-1)) + String.fromCharCode(lastchar)
           }
       } 
...
function convertEnglishConsonantToAmharic(lastChar, amharicValue, shiftKey) {
   ...
   {      //amharic.innerHTML = amharic.innerHTML +"?" ;
       fieldValue = 4637;
